class Solution {
    ...
    public boolean DFS(char [][] board, int k, int j, int count, String word)
    {
        ...
        if(k >= board.length || k < 0 || j >= board[k].length || j < 0 || board[k][j]  != word.charAt(count)) return false;
        
        char temp = board[k][j];
        board[k][j] = ' ';
        boolean result = DFS(board, k+1, j, count+1, word) || DFS(board, k-1, j, count+1, word)
